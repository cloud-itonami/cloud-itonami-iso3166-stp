(ns marketentry.governor-contract-test
  ...
  This is the STP actor's SIX-check governor (see `marketentry.governor`
  ...
  distinct STP tax-ID scheme. `commercial-registration-missing` is the
  STP analog of AGO's `ao-entity-missing` flagship check."
  ...
  (let [[db actor] (fresh)
        res (exec-op actor "t1"
                  {:op :engagement/intake :subject "eng-1"
                   :patch {:id "eng-1" :operator "Príncipe Trading Lda"}} operator)]
    (is (= :commit (get-in res [:state :disposition])))
    (is (= "Príncipe Trading Lda" (:operator (store/engagement db "eng-1"))) "SSoT actually updated")
...
(deftest commercial-registration-missing-is-held-and-unoverridable
  (testing "missing DGRN commercial registration -> HARD hold (STP flagship check)"
    (let [[db actor] (fresh)
      ...
      (is (some #{:commercial-registration-missing} (-> (store/ledger db) last :basis)))
...
(deftest clean-second-engagement-commits-end-to-end
  (testing "eng-4 (a second, differently-shaped clean engagement) also runs assess->draft->submit to commit"
    (let [[db actor] (fresh)
          _ (assess! actor "t12pre" "eng-4")
          _ (draft! actor "t12pre" "eng-4")
          r1 (exec-op actor "t12" {:op :filing/submit :subject "eng-4"} operator)
