...
(defn- exercise [s]
  (store/commit-record! s {:effect :engagement/upsert
                           :value {:id "eng-x" :operator "X Co" :jurisdiction "STP"
  ...
  (store/commit-record! s {:effect :assessment/set
                           :path ["eng-x"]
                           :payload {:jurisdiction "STP" :checklist ["a"] :spec-basis "x"}})
...
(deftest seed-stores-have-the-demo-engagement-set
  (testing "MemStore and DatomicStore seed to the same 4-engagement demo set"
    (let [mem (store/seed-db)
          dat (store/datomic-seed-db)]
      (is (= 4 (count (store/all-engagements mem))))
      (is (= 4 (count (store/all-engagements dat))))
