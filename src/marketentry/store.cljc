(ns marketentry.store
  "SSoT for the STP market-entry compliance actor, behind a `Store`
  ...
  PLUS `:requires-nif?`/`:nif-verified?`): the São Tomé and Príncipe
  dossier (`marketentry.facts`) grounds only DGRN commercial/company
  registration (`:requires-stp-registration?`/`:has-stp-registration?`),
  not a distinct tax-ID/corporate-number scheme -- so there is no STP
  analog of `:requires-nif?`/`:nif-verified?` here. See
...
(defn demo-data
  ...
  4 engagements exercises a distinct governor path:
    ...
    eng-2 -- commercial-registration-missing (requires but lacks DGRN reg.)
    eng-3 -- engagement-fee-mismatch (claimed-fee != base + months x rate)
    eng-4 -- clean, different fee shape (a second honest commit path)"
  []
  {:engagements
   {"eng-1" {:id "eng-1" :operator "Príncipe Trading Lda" :portal "Lei nº 8/2009 filing (no verified e-procurement portal)"
             :base-fee 350000 :monthly-rate 20000 :monitoring-months 12
             :claimed-fee 590000.0
             :requires-stp-registration? true :has-stp-registration? true
             :drafted? false :submitted? false
             :jurisdiction "STP" :status :intake}
    "eng-2" {:id "eng-2" :operator "Golfo Logistics" :portal "Lei nº 8/2009 filing (no verified e-procurement portal)"
             :base-fee 350000 :monthly-rate 20000 :monitoring-months 12
             :claimed-fee 590000.0
             :requires-stp-registration? true :has-stp-registration? false
             :drafted? false :submitted? false
             :jurisdiction "STP" :status :intake}
    "eng-3" {:id "eng-3" :operator "Obo Civic Works" :portal "Lei nº 8/2009 filing (no verified e-procurement portal)"
             :base-fee 350000 :monthly-rate 20000 :monitoring-months 12
             :claimed-fee 750000.0
             :requires-stp-registration? true :has-stp-registration? true
             :drafted? false :submitted? false
             :jurisdiction "STP" :status :intake}
    "eng-4" {:id "eng-4" :operator "Calulu Export Systems" :portal "Lei nº 8/2009 filing (no verified e-procurement portal)"
             :base-fee 200000 :monthly-rate 15000 :monitoring-months 6
             :claimed-fee 290000.0
             :requires-stp-registration? true :has-stp-registration? true
             :drafted? false :submitted? false
             :jurisdiction "STP" :status :intake}}})
...
(defn- engagement->tx [{:keys [id operator portal base-fee monthly-rate monitoring-months claimed-fee
                               requires-stp-registration? has-stp-registration?
                               drafted? submitted?
  ...
  (cond-> {:engagement/id id}
    ...
    (some? requires-stp-registration?)    (assoc :engagement/requires-stp-registration? requires-stp-registration?)
    (some? has-stp-registration?)         (assoc :engagement/has-stp-registration? has-stp-registration?)
...
(def ^:private engagement-pull
  [:engagement/id :engagement/operator :engagement/portal :engagement/base-fee :engagement/monthly-rate
   ...
   :engagement/requires-stp-registration? :engagement/has-stp-registration?
...
(defn- pull->engagement [m]
  (when (:engagement/id m)
    {:id (:engagement/id m) :operator (:engagement/operator m) :portal (:engagement/portal m)
     ...
     :requires-stp-registration? (boolean (:engagement/requires-stp-registration? m))
     :has-stp-registration? (boolean (:engagement/has-stp-registration? m))
