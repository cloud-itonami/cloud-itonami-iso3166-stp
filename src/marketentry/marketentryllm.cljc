(ns marketentry.marketentryllm
  ...
  the STP public-sector market-entry compliance actor.
...
(defn- propose-submit
  ...
  (let [e (store/engagement db subject)]
    {:summary    (str subject " 向け提出提案"
                      (when e (str " (operator=" (:operator e) ")")))
     :rationale  (if e
                   (str "has-stp-registration?=" (:has-stp-registration? e)
     ...
     :confidence (if (and e
                          (or (not (:requires-stp-registration? e))
                              (:has-stp-registration? e)))
                   0.9 0.3)}))
